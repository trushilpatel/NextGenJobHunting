...
type JobPost struct {
	...
	JobID                       string         `gorm:"unique;not null;size:30" json:"jobId"`            // Unique Job ID, up to 30 characters
	JobTitle                    string         `gorm:"not null;size:30" json:"jobTitle"`                // Job title, up to 30 characters
	EmploymentType              EmploymentType `gorm:"type:varchar(10);not null" json:"employmentType"` // Enum for employment type
	SalaryRange                 string         `gorm:"size:30" json:"salaryRange"`                      // Salary range, up to 30 characters
	JobType                     JobType        `gorm:"type:varchar(10);not null" json:"jobType"`        // Enum for job type
	ContractLength              string         `gorm:"size:30" json:"contractLength"`                   // Contract duration, if applicable, up to 30 characters
	Industry                    string         `gorm:"size:30" json:"industry"`                         // Industry related to the job, up to 30 characters
	...
	EducationLevel              string         `gorm:"size:30" json:"educationLevel"`                   // Minimum education level required, up to 30 characters
	...
	IsRequireUSAPerson          bool           `json:"isRequireUSAPerson"`                              // Indicates if the person must be a U.S. citizen or permanent resident
	IsSecurityClearanceRequired bool           `json:"isSecurityClearanceRequired"`                     // Whether security clearance is required
	...
	Hirer                       string         `gorm:"size:20" json:"hirer"`                            // The company/person responsible for hiring, up to 20 characters
